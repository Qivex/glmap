...
class GLMap extends CanvasContext {
	...
	render(time) {
		...
		// Temp for Markers: Simple animation
		for (let layer of this.layers) {
			layer.onPan(64,128)
			layer.onZoom(2*Math.sin(time / 1000) + 5)
		}
		
		
		/*
		// Temp for Tiles: Fixed location
		for (let layer of this.layers) {
			layer.onPan(51,111)	// Uneven numbers to force early break in updateRequiredTiles()
			layer.onZoom(6)
		}
		*/
