...
class TileLayerProgram extends ShaderProgram {
	constructor(context) {
		...
		gl.vertexAttribPointer(this.offsetAttributeLocation, 2, gl.FLOAT, false, 0, 0)
	...
	setTileLocations(tiles) {
		let tileOffsetArray = new Array(tiles.length * 2)
		let tileOrderArray = new Array(tiles.length)
		for (let i = 0; i < tiles.length; i++) {
			tileOffsetArray[2*i]     = tiles[i].x
			tileOffsetArray[2*i + 1] = tiles[i].y
